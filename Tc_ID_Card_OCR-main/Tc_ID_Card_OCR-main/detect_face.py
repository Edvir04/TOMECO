...
class DlibFaceDetector(FaceDetector):        
    ...
    def findFace(self, image):
        if dlib is None:
            raise ImportError("dlib is not installed. Please install it with: pip install dlib")
...
class SsdFaceDetector(FaceDetector):
    ...
    def findFace(self,img):
        ...
        if not os.path.exists(modelFile) or not os.path.exists(configFile):
            # Model files not found, return 0 (no face detected)
            # This is OK - face detection is optional
            return 0
        
        try:
            FaceNet = cv2.dnn.readNetFromCaffe(configFile, modelFile)
        except Exception as e:
            # If model loading fails, return 0 (no face detected)
